...
class DockerRunner(Runner):
    ...
    def __init__(
        self, execution_output_dir: Callable[[Metadata], pl.Path] | None = None
    ) -> None:
        """Create a new DockerRunner.

        Args:
            execution_output_dir: A function that returns the output
                directory for a given Metadata.
        ...
        self.execution_output_dir: Callable[[Metadata], pl.Path] = (
            _default_execution_output_dir
            if execution_output_dir is None
            else execution_output_dir
        )
